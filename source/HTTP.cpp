...
int HTTP::add_listening_socket_to_poll(struct epoll_event &ev, int &ret, int listening_socket) {
...
int HTTP::close_connection(int &cfd, int &epfd, epoll_event &ev, int &ret) {
    // close
    close(cfd);
    if (cfd == -1) {
...
int HTTP::read_socket(struct epoll_event &ev, int &ret) {
    ...
    if (buflen == 0 && this->_inc_msgs[cfd].size() == 0) {
        this->close_connection(cfd, this->_epfd, ev, ret);
    ...
    if (buflen == -1) {
        ...
        if (this->close_connection(cfd, this->_epfd, ev, ret)) {
            this->_inc_msgs.erase(cfd);
           return 1;
        }
    }
...
int HTTP::monitor_multiple_fds() {
    ...
    std::vector<Server *>::iterator it;
    for (it = this->_servers.begin() ; it != this->_servers.end(); ++it)
        this->add_listening_socket_to_poll(ev, ret, (*it)->get_sockfd());
    ...
    while (!g_stop) {
        ...
        for (int i = 0; i < nfds; i++) {
            ...
            if (evlist[i].events & EPOLLIN) {
                ...
                if (this->read_socket(evlist[i], ret))
                    break;
